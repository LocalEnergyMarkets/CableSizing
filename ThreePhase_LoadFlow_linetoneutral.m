...
if isfield(Feeder,'Vpu_slack_phase')
    ...
    Vfslack = Vbase_fase*exp([0,-1i*120*pi/180,1i*120*pi/180]);  % slack 1 < 0?
end
...
% corrientes en las cargas: las corrientes van hacia abajo     
  for k = 1:Feeder.NumC
      ...
      Sdelta    = zeros(3,1);
      if estrella == 1
         Sestrella(1) = Feeder.Loads(k,4) + 1i*Feeder.Loads(k,5);
         Sestrella(2) = Feeder.Loads(k,6) + 1i*Feeder.Loads(k,7);
         Sestrella(3) = Feeder.Loads(k,8) + 1i*Feeder.Loads(k,9);
      else
         Sdelta(1) = Feeder.Loads(k,4) + 1i*Feeder.Loads(k,5);
         Sdelta(2) = Feeder.Loads(k,6) + 1i*Feeder.Loads(k,7);
         Sdelta(3) = Feeder.Loads(k,8) + 1i*Feeder.Loads(k,9);                 
      end
...
%% efecto capacitivo de las lineas
  for k = 1:Feeder.NumL
      ...
      Is(:,N1) = Is(:,N1) + 1i*Bf*Vs(:,N1);
      Is(:,N2) = Is(:,N2) + 1i*Bf*Vs(:,N2);
  end
